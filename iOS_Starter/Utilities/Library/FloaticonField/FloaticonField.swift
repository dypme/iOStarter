...
class FloaticonField: FloaticonFieldEffects {
    ...
    private var insetLeft: CGFloat = 0
    /// Padding right to adjustment with setting of field
    private var insetRight: CGFloat = 0
    
    /// Closure for condition field error
    private var errorCondition: ((String) -> Bool)?
    /// Closure for action with error appear
    private var errorExecute: ((String) -> String?)?
    ...
    @IBInspectable var isFloatingEnable: Bool = true
    /// Boolean value to make field required
    @IBInspectable var isFieldRequired: Bool = false
    ...
    @IBInspectable var icon: UIImage? {
        didSet {
            if let image = icon {
                let minSize: CGFloat = (self.font?.pointSize)!
                self.leftViewMode = .always
                let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: minSize, height: minSize))
                imageView.contentMode = .scaleAspectFit
                imageView.image = image
                self.leftView = imageView
                insetLeft = minSize + 12
            }
        }
    }
    ...
    @IBInspectable dynamic open var borderActiveColor: UIColor? = .black {
        didSet {
            updateBorder()
        }
    }
    ...
    @IBInspectable dynamic open var floatingPlaceholderColor: UIColor = .black {
        didSet {
            updatePlaceholder()
        }
    }
     ...
     */
    @IBInspectable dynamic open var placeholderFontScale: CGFloat = 0.78 {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    @IBInspectable dynamic open var floatingPlaceholder: String? {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    /// Action response that not allow used in field
    private var notAllowedActions: [ResponderStandardEditActions] = [] {
        didSet {
            if !allowedActions.isEmpty && !notAllowedActions.isEmpty {
                allowedActions = []
            }
    ...
    func setError(when condition: @escaping ((String) -> Bool), execute: ((String) -> String?)? = nil) {
        self.errorCondition = condition
        self.errorExecute = execute
    }
    ...
    override open func fieldDidEndEditing() {
        ...
        guard let condition = (errorCondition?(self.text!)), condition && isVisible(view: self) else {
            self.rightView = nil
            insetRight = 0
            updateBorder()
            updatePlaceholder()
        ...
        updateError(text: errorExecute?(self.text!))
        
        let minSize: CGFloat = (self.font?.pointSize)!
        self.rightViewMode = .always
        let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: minSize, height: minSize))
        imageView.contentMode = .scaleAspectFit
        imageView.image = errorIcon ?? UIImage.errorImage(fontSize: minSize, color: self.errorColor)
        self.rightView = imageView
        insetRight = minSize + 12
    }
